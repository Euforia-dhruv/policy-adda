...
const employeeLinks = [
  { href: "/dashboard-employee", label: "Overview", icon: "dashboard" },
  { href: "/dashboard-employee/customers", label: "Assigned Customers", icon: "users" },
  { href: "/dashboard-employee/applications", label: "Applications", icon: "file-text" },
  { href: "/dashboard-employee/follow-ups", label: "Follow-ups", icon: "clock" },
  { href: "/dashboard/support", label: "Support", icon: "message-square" },
  { href: "/dashboard/profile", label: "Profile", icon: "user" },
];

function EmployeeSidebar({ user, pathname }: { user: UserProfile | null; pathname: string }) {
    ...
    <>
      ...
      <div className="border-t border-white/10 p-4">
        <div className="flex items-center gap-3">
          <div className="grid h-10 w-10 place-items-center rounded-full bg-gradient-to-br from-emerald-500 to-emerald-600 text-sm font-medium text-ivory">
            {user?.full_name?.split(" ").map((n) => n[0]).join("").toUpperCase().slice(0, 2) || "E"}
          </div>
...
export default function EmployeeDashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const getUser = async () => {
      ...
      if (profile && !["executive", "support", "manager", "admin"].includes(profile.role)) {
  ...
  return (
    <div className="flex min-h-screen bg-canvas">
      <aside className="hidden lg:flex lg:w-64 lg:flex-col lg:border-r lg:border-white/10 lg:bg-surface">
        <EmployeeSidebar user={user} pathname={pathname} />
      </aside>
      ...
      <div className="flex flex-1 flex-col">
        <header className="flex h-16 items-center justify-between border-b border-white/10 bg-surface px-4 lg:hidden">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="font-medium text-ivory">Employee</span>
          </div>
          <div className="grid h-9 w-9 place-items-center rounded-full bg-gradient-to-br from-emerald-500 to-emerald-600 text-xs font-medium text-ivory">
            {user?.full_name?.split(" ").map((n) => n[0]).join("").toUpperCase().slice(0, 2) || "E"}
          </div>
        </header>
